...
void show_help(int use_ncurses)
{
    ...
    const char *help_text[] = {
        ...
        "  csvview +N <file>               Open file and jump to row N (e.g. +42)",
        "",
        "File format auto-detection:",
        "  .csv -> comma   .tsv -> tab   .psv -> pipe   .ecsv -> space   other -> use --sep",
        "",
        "Key Bindings (main view):",
        "  Arrows / hjkl        Move cursor",
        "  Nj / Nk              Move N rows down / up (vim-style: type number then j or k)",
        "  PageUp / PageDown    Scroll one page",
        "  Home / End / K / J   First / last visible row",
        "  gg                   Jump to first row",
        "  G                    Jump to last row",
        "  H / L                Jump to first / last visible column",
        "  /                    Search — highlights persist while navigating; rows marked with *",
        "  n / N                Next / previous search result",
        "  &                    Filter table to search matches only (toggle; works across columns)",
        "  R                    Reset filter AND clear search highlights",
        "  :noh                 Clear search highlights (like vim :nohlsearch)",
        "  f                    New filter [Tab = autocomplete column]",
        "  Shift+F              Edit current filter (pre-fills active filter for editing)",
        ...
        "  Z                    Freeze / unfreeze rows up to current row (Shift+Z)",
        "  v                    Row detail view — full row as column/value table with word wrap",
        ...
        "  Space                Toggle mark column for graph (numeric only, underlined when marked)",
        ...
        "  :hide <glob>         Hide columns matching glob (e.g. :hide *Forecast)",
        "  :show <glob>         Show columns matching glob (:show * = show all)",
        "  :type <glob> <t>     Set type for matching cols (t = num | str | date)",
        "  :fmt <glob> <f>      Set format for matching cols (f = auto | N | date-pattern)",
        ...
        "",
        "Bookmarks (vim-style):",
        "  m                    Auto-set row bookmark (next free letter a-z)",
        "                       Press m again on the same row -> clears it",
        "  m<a-z>               Explicit letter: set / clear row bookmark",
        "  '<a-z>               Jump to row bookmark (if hidden by filter: prompts to clear)",
        "  M                    Auto-set column bookmark (next free letter a-z)",
        "  M<a-z>               Explicit letter: set / clear column bookmark",
        "  \"<a-z>               Jump to bookmarked column",
        ...
        "  Gutter column shows bookmark letter (or * for search match) next to line number",
        "",
        "Theme:",
        "  :theme <name>        Switch colour theme (saved to ~/.config/csvview/config)",
        "  :theme               List available themes",
        "  Themes: dark, light, tokyonight, nord, catppuccin,",
        "          solarized_light, paper, turbovision, dracula, gruvbox",
        ...
        "  [f]                  Open full frequency list (all unique values)",
        "    j / k / ↑↓         Navigate values",
        "    s                  Cycle sort: count desc → count asc → A→Z → Z→A",
        ...
        "    arrows / hjkl      Navigate the matrix",
        "    Enter              Launch scatter plot for selected pair",
        "    q / Esc            Close",
        "  :outliers [sigma]    Outlier report — rows with |z-score| >= sigma (default 3.0)",
        "    j / k              Navigate outlier list",
        "    Enter              Jump to that row in the table",
        "    f                  Filter table to that exact value",
        "    q / Esc            Close",
        "",
        "Pivot table (p / P):",
        ...
        "  Filter by            Report filter — select column as 3rd dimension",
        "  T                    Transpose: swap rows and columns instantly",
        "  v                    Cycle display mode: Normal / % Row / % Col /",
        "                       % Grand / Rank / Running Total",
        "  S                    Toggle braille sparklines next to row labels",
        "  F                    Open report filter popup (multi-select with *)",
        ...
        "  :em [file]           Export pivot to Markdown (default pivot.md)",
        "  :ptop N              Show only top N rows by value",
        "  :pbot N              Show only bottom N rows by value",
        "  :pfilter OP VAL      Filter rows: >1000, >=500, <100, !=0",
        "  :pclear              Remove pivot row filter",
        "  :q                   Quit pivot mode",
        "  :o                   Reopen settings",
        ...
        "",
        "Graph mode:",
        "  Space                Mark / unmark current column for multi-series graph",
        "                         (marked columns shown with underline in header)",
        "  Ctrl+G               Open graph: all marked columns + current column",
        ...
        "  Status bar shows current graph type and Y-scale (e.g. line  Y:linear)",
        "",
        ...
        "                         Works in both regular and scatter (:gsc) mode",
        ...
        "                         ← → / h l  pan X axis; ↑ ↓ / k j  pan Y axis",
        "                         + / =  zoom in; -  zoom out; 0  reset to full view",
        "                         Status bar shows [zoom] when viewport is active",
        ...
        "                         Log scale export correctly uses min positive value as base",
        "  :gpie [col]          Pie/donut chart: value frequencies for the given column",
        "  :gbox [col...]       Box-and-whisker plot: min/Q1/median/Q3/max per column",
        "  :gheat               Correlation heatmap: Pearson r for all numeric columns",
        "  :gr [row1 row2 ...]  Row graph: plot row values across columns (numeric only)",
        "                         :gr          — graph current row",
        "                         :gr 3 5 8    — overlay rows 3, 5, 8",
        "                         Supports line/bar/dot, zoom, cursor, 1-9 toggle series",
        "",
        "Sparklines:",
        "  :spark               Toggle sparkline column (braille trend per row)",
        "  :spark on/off        Explicit on/off (saved in .csvf)",
        "                         Shows trend of numeric values across columns",
        "                         Positioned left, between row number and data",
        "",
